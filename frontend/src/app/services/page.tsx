    ...
    <main className="flex-1 bg-[#f7f7f7]">
      ...
      <div className="bg-white border-b border-gray-300 w-full">
        <div className="container mx-auto flex">
          <div className="w-full md:w-[250px] bg-black text-white py-6 px-6 flex flex-col items-center justify-center">
             <span className="text-[10px] font-bold uppercase tracking-widest">CLINICAL SERVICES</span>
          </div>
          <div className="flex-1 flex justify-around">
            <button onClick={() => setActiveService("General Surgery")} className="flex-1 border-r border-gray-300 flex flex-col items-center justify-center p-4 hover:bg-gray-50 cursor-pointer transition-colors">
               <span className="text-[10px] font-bold uppercase tracking-widest text-black">DIAGNOSTICS</span>
            </button>
            <button onClick={() => setActiveService("General Surgery")} className="flex-1 border-r border-gray-300 flex flex-col items-center justify-center p-4 hover:bg-gray-50 cursor-pointer transition-colors">
               <span className="text-[10px] font-bold uppercase tracking-widest text-black">SURGERY</span>
            </button>
            <button onClick={() => setActiveService("Maternity")} className="flex-1 flex flex-col items-center justify-center p-4 hover:bg-gray-50 cursor-pointer transition-colors">
               <span className="text-[10px] font-bold uppercase tracking-widest text-black">MATERNITY</span>
            </button>
          </div>
        </div>
      </div>

      <div className="container mx-auto flex flex-col md:flex-row min-h-[calc(100vh-100px)]">
        
        {/* Sidebar Navigation */}
        <aside className="w-full md:w-[250px] bg-[#fcfcfc] border-r border-gray-300 flex-shrink-0">
          <div className="p-6 border-b border-gray-300">
            <h2 className="text-3xl font-black tracking-tighter mb-2">Services</h2>
            <p className="text-xs text-gray-600">Comprehensive care across all major medical specialties.</p>
          </div>
          ...
          <div className="space-y-0">
            {(Object.keys(servicesData) as ServiceKey[]).map((key) => (
              <button 
                ...
                className={`w-full text-left block p-6 border-b border-gray-200 cursor-pointer group flex justify-between items-center transition-colors ${
                  activeService === key ? "bg-black text-white" : "hover:bg-gray-50 bg-[#fcfcfc] text-black"
                }`}
              >
                <span className="text-xs font-bold uppercase tracking-widest">{key}</span>
                <span className={`font-bold ${activeService === key ? "text-white" : "text-gray-400"}`}>→</span>
              </button>
            ))}
          </div>
        </aside>
        ...
        <div className="flex-1 bg-white">
          ...
          <div className="h-[400px] w-full bg-gray-200 relative overflow-hidden border-b border-gray-300 animate-in fade-in duration-500" key={service.title + "-img"}>
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-12">
               <span className="bg-[#eadd7b] text-black text-[10px] font-bold uppercase tracking-widest px-3 py-1 mb-4 w-max">{service.department}</span>
               <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-white leading-none">{service.title}</h1>
            </div>
          </div>

          <div className="p-8 md:p-12 lg:p-16 animate-in slide-in-from-bottom-4 fade-in duration-500" key={service.title + "-content"}>
            
            <p className="text-lg md:text-xl font-medium leading-relaxed mb-12 max-w-3xl">
              {service.description}
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-12 mb-16">
              ...
              <div>
                <h2 className="text-2xl font-black tracking-tighter mb-8 uppercase">Key Benefits & Features</h2>
                <div className="space-y-6">
                  
                  {service.benefits.map((benefit, i) => (
                    <div key={i} className="flex gap-6 items-start">
                      <div className="w-8 h-8 rounded-full border-2 border-black flex items-center justify-center font-black flex-shrink-0 mt-1">{i + 1}</div>
                      <div>
                        <h3 className="font-bold text-sm uppercase tracking-widest mb-2">{benefit.title}</h3>
              ...
              <div className="bg-[#fcfcfc] border border-gray-300 p-8 md:p-12 flex flex-col justify-center">
                <span className="text-4xl mb-6">"</span>
                <h3 className="text-xl font-black tracking-tight leading-relaxed mb-8">
                  {service.quote}
                </h3>
                <Link href="/book" className="mt-auto inline-block text-center bg-black text-white px-8 py-4 text-[10px] font-bold uppercase tracking-widest hover:bg-gray-800 transition-colors self-start shadow-md">
